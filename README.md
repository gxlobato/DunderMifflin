...
A ideia é que cada etapa seja construída (e documentada) separadamente, começando pela logística — a primeira parte do negócio implementada — passando agora por vendas, e crescendo aos poucos para as demais áreas.
...
- **Funcionários** são os personagens da série (Jim Halpert, Dwight Schrute, Michael Scott, Pam, Kevin, Creed, e por aí vai), cada um em uma área da empresa (Vendas, Gerência, Administrativo, Contabilidade, Armazém, RH) e com um cargo (Junior, Pleno, Sênior, Sub-Gerente, Gerente) — um espelho dos papéis que eles tinham na Dunder Mifflin.
...
- **Dwight Schrute e Jim Halpert**, os vendedores mais icônicos da série, também são os que mais vendem na simulação — uma homenagem direta à rivalidade (e talento) dos dois na área de vendas.
...
Evoluiu de uma tabela de apoio (usada só para alimentar datas de saída da logística) para uma área própria. Cada venda sorteia produto, cliente e vendedor — com pesos que fazem Dwight e Jim se destacarem como os maiores vendedores, seguidos pelo restante da área de Vendas, mas mantendo a possibilidade de qualquer funcionário vender. Também foi adicionada uma regra de desconto escalonada por valor da venda, área e cargo do vendedor, além do rastreamento de custo (`valor_custo_total`) para permitir análise de margem.

📄 Resumo técnico: [`src/vendas/README.md`](src/vendas/README.md)
...
O quadro de funcionários já tem nome, área, cargo e data de contratação — inclusive uma primeira função de faixas de comissão por cargo (`monta_comissao`), ainda não conectada às demais áreas. O plano é ir além: turnover, avaliação de desempenho, folha de pagamento, plano de carreira.

### 📊 Contabilidade — *ainda a ser implementado*

Consolidar os dados de vendas, frete e folha de pagamento em uma visão financeira: receita, custos operacionais (incluindo o custo de frete já mapeado por UF na logística, e agora também o custo de produto trazido por `valor_custo_total` em vendas), margem por produto e por rota.
...
│   │
│   ├── vendas/                      # etapa implementada
│   │   ├── __init__.py
│   │   ├── vendas.py                # montar_vendas
│   │   ├── comissao.py              # monta_comissao (calculada, ainda não conectada a vendas)
│   │   └── main.py                  # orquestra as funções da etapa (lógica pura, sem gerar arquivos)
│   │
...
│   │                                # das camadas (raw/source/cache/logistica/vendas)
│   ├── raw/
│   ├── logistica/
│   └── vendas/
│
...
- **`shared/`** evita duplicar código entre áreas. Clientes e funcionários, por exemplo, não são exclusivos da logística — vendas e contabilidade também precisam deles, então centralizar evita reimplementação. `monta_funcionarios` agora inclui `Cargo`, informação usada tanto por vendas (regra de desconto) quanto pela futura área de RH (comissão).
...
- **`tests/`** antecipa o item de testes automatizados já previsto no roadmap da logística, agora também estendido a vendas.
- **Um README por módulo.** Cada pasta de área de negócio (`logistica/`, `vendas/`, e futuramente `qualidade/`, `rh/`, `contabilidade/`) tem seu próprio `README.md` com o detalhamento técnico daquela etapa — lógicas aplicadas, escolhas de design, bugs encontrados e próximos passos. Este README na raiz mantém só um resumo de cada etapa e um link para o detalhamento.
